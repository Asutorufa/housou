...
fn get_cache_key(req: &MetadataRequest, host: &str) -> String {
    ...
    if let Some(ref id) = req.tmdb_id {
        params.append_pair("tmdb_id", id);
    }
    if let Some(ref id) = req.mal_id {
        params.append_pair("mal_id", id);
    }
    if let Some(ref id) = req.anilist_id {
        params.append_pair("anilist_id", id);
    }
    if let Some(y) = req.year {
        params.append_pair("year", &y.to_string());
    }
    ...
    format!("https://{}/api/metadata?{}", host, params.finish())
}
